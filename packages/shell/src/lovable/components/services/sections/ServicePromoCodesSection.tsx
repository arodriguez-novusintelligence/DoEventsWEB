...
import type { ServiceFormData, ServicePromoCodeBatch } from '@lovable/data/servicesData';
...
interface ServicePromoCodesSectionProps {
  formData: ServiceFormData;
  updateForm: (partial: Partial<ServiceFormData>) => void;
}

const ServicePromoCodesSection = ({ formData, updateForm }: ServicePromoCodesSectionProps) => {
  const batches = formData.promoCodes ?? [];
  const enabled = formData.promoEnabled ?? batches.length > 0;
  ...
  const handleToggle = (next: boolean) => {
    updateForm({ promoEnabled: next, ...(next ? {} : { promoCodes: [] }) });
  };
  ...
  const addBatch = () => {
    ...
    const codes = generateUniquePromoCodes(quantity, existing, 'DOS');
    const batch: ServicePromoCodeBatch = {
      id: `service-promo-${Date.now()}`,
    ...
    updateForm({ promoCodes: [...batches, batch], promoEnabled: true });
    setDraft({ currency: 'COP', value: '', quantity: '10', description: '' });
    setOpenBatch(batch.id);
    toast.success(`${quantity} códigos generados para reservas del servicio`);
  };
    ...
    <div className="min-w-0 space-y-4 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        >
          ...
          <span>
            <span className="block text-sm font-semibold text-foreground">Códigos promocionales del servicio</span>
            <span className="block text-xs text-muted-foreground">
              Genera códigos con descuento para que tus clientes los rediman al reservar el servicio.
            </span>
          </span>
        </button>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Ej. Descuento de lanzamiento para reservas de fin de semana."
                  className="mt-1 w-full rounded-xl border border-border bg-card px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none"
                />
              </div>
            </div>
...
export default ServicePromoCodesSection;
