...
export function PlaceFormProvider({
  ...
  const completedCount = useMemo(() => {
    ...
    if (form.pricing.perDay || form.pricing.perWeek || form.pricing.perMonth || form.pricing.perMultiDay) count += 1;
    if (form.globalStartTime && form.globalEndTime) count += 1;
  ...
  const missingRequiredFields = useMemo(() => {
    ...
    if (form.hasSeating && !(Number(form.capacity) > 0)) missing.push('Capacidad del lugar');
    if (form.hasSeating && (form.gates.length === 0 || !form.floors.some((f) => f.categories.length > 0))) {
      missing.push('Mapa de silletería y puertas');
    }
    if (!(form.pricing.perDay || form.pricing.perWeek || form.pricing.perMonth || form.pricing.perMultiDay)) {
      missing.push('Al menos un precio de reserva');
    }
    if (!form.globalStartTime || !form.globalEndTime) missing.push('Horario de disponibilidad');
