...
export interface WompiBuyerDetails {
  name: string;
  email: string;
  phone: string;
}
...
export interface WompiMockPaymentViewProps {
  ...
  disabled?: boolean;
  onConfirmPayment: (buyer: WompiBuyerDetails) => Promise<void>;
  onAbort?: () => void;
  onPayLater?: () => void;
  onOpenMenu?: () => void;
  defaultBuyer?: {
...
export const WompiMockPaymentView: React.FC<WompiMockPaymentViewProps> = ({
  ...
  const isSandbox = useMemo(() => {
    ...
    return envName === 'devaws' || envName === 'dev' || envName === 'qa';
  }, []);
  ...
  if (processing) {
    return (
      <div className="mx-auto flex min-h-screen max-w-lg flex-col items-center justify-center space-y-5 bg-background px-6">
  ...
  return (
    <div className="mx-auto min-h-screen max-w-lg bg-[#EEF0FB] pb-32">
      {onOpenMenu && (
        <div className="hidden px-4 pt-4 lg:flex">
          <button
            type="button"
            onClick={onOpenMenu}
      ...
      {isSandbox && (
        <div
          className="flex justify-center bg-rose-100 py-2"
          style={{ backgroundImage: 'repeating-linear-gradient(45deg, rgba(244,63,94,0.08) 0 8px, transparent 8px 16px)' }}
        >
          <span className="rounded bg-rose-600 px-3 py-1 text-[11px] font-bold tracking-wider text-white">
            MODO DE PRUEBAS
          </span>
        </div>
      )}
      ...
      <div className="bg-emerald-50/40 px-5 py-6 text-center">
        ...
        <button
          type="button"
          className="mt-3 inline-flex items-center gap-2 rounded-full border border-border bg-background px-4 py-1.5 text-xs font-medium"
        >
      ...
      <div className="mt-5 px-4">
        <p className="mb-2 text-sm font-bold">Estás pagando con:</p>
      ...
      <div className="mt-5 space-y-4 px-4">
        ...
        <div>
          ...
          <div className="mt-1 grid grid-cols-[80px_1fr] gap-2">
            <div className="flex items-center justify-between rounded-md border border-border bg-background px-2 text-sm">
              +57 ▾
            </div>
            <Input
              value={buyerPhone}
        ...
        <p className="text-center text-xs text-muted-foreground">
          ⊜ Grupo <span className="font-bold">Cibest</span>
        </p>
      </div>

      {/* Footer: Pagar más tarde + No deseo continuar */}
      {onAbort && (
        <div className="fixed bottom-0 left-0 right-0 z-30 px-4 py-3">
          <div className="mx-auto max-w-lg bg-[#EEF0FB]">
          {onPayLater && (
            <button
              type="button"
              onClick={onPayLater}
