      ...
      <div className="px-4 pt-4">
        {loading ? (
          <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-border/60 bg-card py-12 shadow-sm">
            ...
            <p className="text-sm font-extrabold text-foreground">Cargando invitaciones…</p>
          </div>
        ) : loadError ? (
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-border/60 bg-card p-10 text-center shadow-sm">
            ...
            <p className="text-sm font-extrabold text-foreground">No se pudieron cargar las invitaciones</p>
            <p className="text-xs font-extrabold text-muted-foreground max-w-[240px]">{loadError}</p>
            {onRetry && (
              <Button type="button" variant="outline" size="sm" className="rounded-full font-extrabold shadow-sm gap-1.5" onClick={onRetry}>
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-primary/25 bg-card p-10 text-center shadow-sm ring-1 ring-primary/10">
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-primary/10 ring-2 ring-primary/20">
            <Ticket className="h-7 w-7 text-primary" />
          </div>
          <p className="text-sm font-extrabold text-foreground">Sin invitaciones</p>
          <p className="text-xs font-extrabold text-muted-foreground max-w-[240px]">
        ...
        <div className="space-y-4">
          {invitations.map((inv) => (
            <button
              ...
              className="flex w-full items-stretch gap-0 rounded-2xl border border-border/60 bg-card shadow-sm text-left overflow-hidden transition-colors hover:border-primary/30 active:scale-[0.99] ring-1 ring-primary/10"
            >
              {inv.image ? (
                <img src={inv.image} alt={inv.title} className="h-36 w-28 flex-shrink-0 object-cover ring-2 ring-primary/20" />
              ) : (
                <div className="flex h-36 w-28 flex-shrink-0 flex-col items-center justify-center gap-1 bg-primary/5 text-primary ring-2 ring-primary/20 rounded-xl">
                  ...
                  <span className="text-[10px] font-extrabold">Evento</span>
                </div>
              )}
              <div className="flex-1 min-w-0 py-3 pr-3">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-base font-extrabold text-foreground leading-snug flex-1">{inv.title}</h3>
                  <span
                    role="button"
                    tabIndex={0}
                    onClick={(e) => {
                ...
                <p className="text-xs font-extrabold text-foreground mt-2 font-mono">
                  {new Date(inv.receivedAt).toLocaleString('es-CO')}
                </p>
                <p className="text-xs font-extrabold text-muted-foreground mt-3">
                  {inv.inviter ? `Invitado por ${inv.inviter}` : 'Invitación a evento'}
                </p>
                <span className={`inline-block mt-2 rounded-full px-2.5 py-0.5 text-[11px] font-extrabold shadow-sm ${statusBadgeClass[inv.status]}`}>
