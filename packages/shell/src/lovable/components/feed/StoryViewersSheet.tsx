  ...
  <Sheet open={open} onOpenChange={onOpenChange}>
    <SheetContent side="bottom" className="max-w-lg mx-auto max-h-[90dvh] rounded-t-2xl border border-border/60 shadow-sm">
      ...
      {loading ? (
        <div className="mt-6 space-y-3">
          {SKELETON_ROWS.map((row) => (
            <div key={row} className="flex items-center gap-3 rounded-2xl border border-border/60 bg-card p-3 shadow-sm animate-pulse">
              <div className="h-10 w-10 rounded-full bg-muted ring-2 ring-primary/20" />
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-primary/25 border-border/60 bg-muted/30 p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-xs font-extrabold text-muted-foreground max-w-xs mx-auto leading-relaxed">
            {storyId
