...
const EventCard = ({
  ...
  >
    ...
    <div className="p-3.5">
      ...
      {event.description && (
        <p className="mt-2 text-xs font-extrabold text-foreground/80 line-clamp-2">{event.description}</p>
      )}
    </div>
  </button>
);
...
const UpcomingEventRow = ({
  ...
  >
    ...
    <FavoriteHeartButton
      active={isFavorite}
      onToggle={() => { if (event.id) onToggleFavorite?.(event.id); }}
      className="shrink-0 self-start flex h-9 w-9 items-center justify-center rounded-full bg-background/90 text-primary shadow-sm ring-2 ring-primary/20"
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-start justify-between gap-2">
        ...
        <FavoriteHeartButton
          active={isFavorite}
          onToggle={() => { if (event.id) onToggleFavorite?.(event.id); }}
          className="shrink-0 flex h-8 w-8 items-center justify-center rounded-full bg-background/90 text-primary shadow-sm ring-2 ring-primary/20"
          iconClassName="h-4 w-4"
        />
      </div>
  ...
  <div className="min-w-[180px] max-w-[180px] flex-shrink-0 rounded-2xl bg-card shadow-sm overflow-hidden border border-border/60 text-left">
    ...
    >
      <div className="relative flex flex-col items-center px-4 pt-5 pb-3">
        ...
        <p className="mt-1 text-xs font-extrabold text-muted-foreground text-center line-clamp-1">
  ...
  return (
    <div className="mx-auto max-w-lg pb-40 bg-background">
      ...
      {discoverLoading && !isInitialDiscoverLoad && (
        <div className="flex items-center justify-center gap-2 border-b border-border/60 bg-muted/40 px-4 py-2 text-xs font-extrabold text-muted-foreground">
